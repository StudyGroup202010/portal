...
public class InputPositionForm {
    ...
    @Size(min = 5, max = 5, message = "{length_check_2}") // 桁数指定
    private String position_cd; // 役職CD
    
    // 必須入力
    @NotBlank(message = "{require_check}")
    @Size(min = 1, max = 50, message = "{length_check}") // 桁数指定
    private String position_name; // 役職名
    
    // 必須入力
    @NotBlank(message = "{require_check}")
    @Size(min = 6, max = 6, message = "{length_check_2}") // 桁数指定
    ...
    @Size(max = 100, message = "{length_check_3}")
