...
public class EmployeeListXlsxView extends AbstractXlsxView {
    ...
    protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
        ...
        row.createCell(18).setCellValue("最終学歴");
        row.createCell(19).setCellValue("学科");
        row.createCell(20).setCellValue("卒業年月");
        row.createCell(21).setCellValue("メールアドレス");
        row.createCell(22).setCellValue("入社日");
        row.createCell(23).setCellValue("退社日");
        row.createCell(24).setCellValue("社員属性ID");
        row.createCell(25).setCellValue("備考");
        row.createCell(26).setCellValue("作成者");
        ...
        for (int i = 0; i < employeeListCount; i++) {
            ...
            // 最終学歴
            row.createCell(18).setCellValue(employeeList.get(i).getFinal_education());
            // 学科
            row.createCell(19).setCellValue(employeeList.get(i).getDepartment());
            // 卒業年月
            row.createCell(20).setCellValue(employeeList.get(i).getGraduation_date());
            // メールアドレス
            ...
            // 備考
            row.createCell(25).setCellValue(employeeList.get(i).getBiko());
            // 作成者
            row.createCell(26).setCellValue(employeeList.get(i).getInsert_user());
            // 作成日時
            String Insert_date = DateUtils
                    .getStringFromDateTimeFormat(employeeList.get(i).getInsert_date().toLocalDateTime());
            row.createCell(27).setCellValue(Insert_date);
            // 更新者
            row.createCell(28).setCellValue(employeeList.get(i).getUpdate_user());
            // 更新日時
            if (employeeList.get(i).getUpdate_date() != null) {
                ...
                row.createCell(29).setCellValue(Update_date);
            }
        }
        // カラム幅を自動調整
        for (int i = 0; i <= 29; i++) {
